var DEBUG = true,
	...
	W = canvas.width = uiCanvas.width = 800,
	H = canvas.height = uiCanvas.height = 600,

	PI2 = 2 * Math.PI;

var vendorPrefix = (function(ua) {
    var ret = "";
    if (/webkit/.test(ua)) {
        ret = "webkit";
...
// Credit: HyperJS source code.
var mobile = (function(b) {
	var a = false;
...
if (DEBUG) {
	...
	params.open();
	params.add(settings, "score").onChange(function(v) {
		score = v;
	});
	params.add(settings, "wave", 0, 10).step(1).onChange(function(v) {
		waveIndex = v;
	});
